...
(function() {
    ...
    function extractSenderMessage(node) {
        ...
        return { sender, message, fullText: fullText.trim() };
    }
    ...
    if (chatBodyDiv) {
        const seenMessages = new Set(Array.from(chatBodyDiv.querySelectorAll('.inMessage')).map(node => extractSenderMessage(node).fullText));
                ...
                if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                    mutation.addedNodes.forEach(node => {
                        if (node.classList?.contains('inMessage')) {
                            const { fullText } = extractSenderMessage(node);
                            if (!seenMessages.has(fullText)) {
                                seenMessages.add(fullText);
                                hasNew = true;
                                handleNewMessage(node, readAloudChatBodyEnabled);
                            }
                        }
                    });
                }
            });
            if (hasNew) bellSound.play().catch(error => console.log('Error playing sound:', error));
