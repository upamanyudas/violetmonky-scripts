// ==UserScript==
// @name         TWB OBS Web Cleanup
// @icon         data:image/png;base64,
// @namespace    https://github.com/upamanyudas/violetmonky-scripts
// @version      0.4
// @description  Remove unwanted buttons/elements from OBS Web and add a responsive floating window overlay
// @match        https://obs-web.niek.tv/*
// @author       Upamanyu Das
// @grant        none
// @homepage     https://github.com/upamanyudas/violetmonky-scripts
// ==/UserScript==

(function () {
  'use strict';

  const removalPhrases = ["⌘", "⇊"];
  const elementConfigs = [{ selector: "button", attr: "title" }];

  function shouldRemove(text) {
    return removalPhrases.some(p => text?.includes(p));
  }

  function removeUnwanted() {
    elementConfigs.forEach(c => {
      document.querySelectorAll(c.selector).forEach(el => {
        if (shouldRemove(el.getAttribute(c.attr))) {
          const li = el.closest("li");
          (li || el).remove();
        }
      });
    });
  }

  function createFloatingSFX() {
    const overlay = document.createElement('div');
    overlay.id = 'twb-sfx-overlay';
    overlay.style.cssText = `
      position:fixed;bottom:20px;right:20px;width:250px;height:144px;
      background:#fff;border:1px solid #ccc;border-radius:8px;
      box-shadow:0 4px 12px rgba(0,0,0,.3);z-index:10000;overflow:hidden;
      resize:both;min-width:200px;min-height:150px;
    `;

    const header = document.createElement('div');
    header.style.cssText = `background:#f1f1f1;padding:5px;cursor:move;
      display:flex;justify-content:space-between;align-items:center;
      border-bottom:1px solid #ccc;`;
    header.innerHTML = `<span style="padding-left:10px;">TWB OBS Soundboard</span>`;
    const close = document.createElement('button');
    close.textContent = '×';
    close.style.cssText = `background:none;border:none;font-size:18px;cursor:pointer;`;
    close.onclick = () => overlay.remove();
    header.appendChild(close);

    const iframe = document.createElement('iframe');
    iframe.src = 'https://twb-obs-sfx.dmg.li/';
    iframe.style.cssText = 'width:100%;height:calc(100% - 30px);border:none;';

    overlay.append(header, iframe);
    document.body.appendChild(overlay);

    // simple drag
    let drag = false, x, y, ix, iy;
    header.onmousedown = e => { drag = true; ix = e.clientX - x; iy = e.clientY - y; };
    document.onmousemove = e => { if (drag) { x = e.clientX - ix; y = e.clientY - iy; overlay.style.left = x + 'px'; overlay.style.top = y + 'px'; overlay.style.right = overlay.style.bottom = 'auto'; } };
    document.onmouseup = () => drag = false;

    // snap back to bottom-right when not dragging
    const snap = () => { if (!drag) { overlay.style.left = overlay.style.top = 'auto'; overlay.style.right = '20px'; overlay.style.bottom = '20px'; } };
    snap();
    window.addEventListener('resize', snap);
  }

  // ←←← Audio Monitors (remote) ←←←
  function addAudioMonitors() {
    const placeholder = document.querySelector('.navbar-item .buttons');
    if (!placeholder) return;

    const container = document.createElement('div');
    container.style.cssText = 'display:inline-block;';

    const iframe = document.createElement('iframe');
    iframe.src = 'https://twb-obs-sfx.dmg.li/audio-monitor.html';
    iframe.style.cssText = 'width:230px;height:57px;border:none;border-radius:6px;';
    iframe.title = 'Audio Monitors';
    iframe.scrolling = 'no';

    container.appendChild(iframe);
    placeholder.insertBefore(container, placeholder.querySelector('button[title="Start Stream"]'));
  }

  function waitForUI() {
    if (document.querySelector('button[title="Disconnect"]')) {
      console.log('OBS UI ready → cleanup + SFX + monitors');
      removeUnwanted();
      createFloatingSFX();
      addAudioMonitors();
    } else {
      setTimeout(waitForUI, 800);
    }
  }

  waitForUI();
})();
